...
SYSTEM_PROMPT = """
你是学校学生智能辅导 Agent。
你可以调用内部工具完成身份验证、课程记录查询、学习记录调取。
涉及学生隐私数据时，优先完成身份验证。
回答必须基于工具结果，不要编造课程记录或学习信息。
当用户要求“最近一次”“最新一次”学习记录时，使用 get_student_learning_sessions，并传入 limit=1。
不要臆造不存在的工具名称。
...
仅输出 JSON，不要输出 markdown，不要解释：
{
  ...
  "tool_sequence": ["verify_student_identity", "get_student_learning_sessions"],
  "steps": ["步骤1", "步骤2"],
...
class QwenMCPAgent:
    ...
    def _merge_plan_candidates(
        ...
        return {
            "objective": candidates[0].get("objective", "完成学生问题回答"),
    ...
    def _build_tool_registry(self) -> Dict[str, ToolHandler]:
        return {
            "verify_student_identity": lambda **kwargs: mcp_tool_service.verify_student(
                self.db, **kwargs
            ),
            "get_student_profile": lambda **kwargs: mcp_tool_service.get_student_profile(
                self.db, **kwargs
            ),
            "get_student_course_records": (
                lambda **kwargs: mcp_tool_service.get_student_course_records(
                    self.db, **kwargs
                )
            ),
            "get_student_learning_sessions": (
                lambda **kwargs: mcp_tool_service.get_student_learning_sessions(
                    self.db, **kwargs
                )
            ),
        }
        ...
        return [
            {
                ...
                "function": {
                    ...
                    "description": "校验学生身份，可通过 student_code 搭配 phone 或 id_number 验证。",
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            },
            {
                "type": "function",
                "function": {
                    "name": "get_student_course_records",
                    "description": "查询学生课程记录信息。",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "student_id": {"type": "integer"},
            ...
            },
            {
                "type": "function",
                "function": {
                    "name": "get_student_learning_sessions",
