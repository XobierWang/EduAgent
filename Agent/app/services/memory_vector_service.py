...
def replace_memory_events(
    ...
    if not is_available():
        logger.info("FAISS unavailable, skip vector write for student_id=%s", student_id)
        return

    VECTOR_DIR.mkdir(exist_ok=True)
    metadata = _load_metadata(student_id)
    embedding_dimensions = _embedding_dimensions()
    logger.info(
        "Writing memory event vectors for student_id=%s source_types=%s event_count=%s dir=%s model=%s dimensions=%s",
        student_id,
        source_types or ["all"],
        len(events),
        VECTOR_DIR,
    ...
    _save_metadata(student_id, metadata)
    _rebuild_index(
    ...
    logger.info(
        "Memory event vectors written for student_id=%s index=%s metadata=%s total_vectors=%s",
        student_id,
        _index_path(student_id),
        _metadata_path(student_id),
        len(metadata),
    )
...
def search_memory_events(
    ...
    if not is_available():
        logger.info("FAISS unavailable, skip vector search for student_id=%s", student_id)
        return []
    if not query.strip():
        return []
    ...
    metadata = _load_metadata(student_id)
    if not metadata:
        logger.info("No vector metadata found for student_id=%s", student_id)
        return []
    ...
    if not index_path.exists():
        ...
        logger.info("Rebuilt missing FAISS index for student_id=%s at %s", student_id, index_path)
    ...
    if index.d != embedding_dimensions:
        ...
        index = faiss.read_index(str(index_path))
        logger.info(
            "Rebuilt FAISS index for student_id=%s due to dimension change, new_dimensions=%s",
            student_id,
            embedding_dimensions,
        )
    ...
    logger.info(
        "Vector search complete for student_id=%s query=%r top_n=%s hit_count=%s",
        student_id,
        query,
        top_n,
        len(rows),
...
def _rebuild_index(
    ...
    if metadata:
        ...
        index.add(matrix)
    faiss.write_index(index, str(_index_path(student_id)))


def _metadata_path(student_id: int) -> Path:
    return VECTOR_DIR / f"student_{student_id}.json"


def _index_path(student_id: int) -> Path:
    return VECTOR_DIR / f"student_{student_id}.index"


def _load_metadata(student_id: int) -> list[dict[str, Any]]:
    path = _metadata_path(student_id)
    if not path.exists():
        return []
    return json.loads(path.read_text(encoding="utf-8"))


def _save_metadata(student_id: int, metadata: list[dict[str, Any]]) -> None:
    _metadata_path(student_id).write_text(
